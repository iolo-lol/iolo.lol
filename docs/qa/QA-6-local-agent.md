...
| SPEC-6 REQ-004 | Evidence records `orchestrator` distinguishing local-agent from github-actions | data | inspect `runs/*.json` from local and prior Actions runs | `orchestrator` field differs correctly | pass | engine `runs/` |
